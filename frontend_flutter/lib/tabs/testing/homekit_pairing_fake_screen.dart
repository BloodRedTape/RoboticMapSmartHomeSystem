...
class HomeKitPairingFakeScreen extends StatefulWidget {
  const HomeKitPairingFakeScreen({super.key});
  @override
  State<HomeKitPairingFakeScreen> createState() => _HomeKitPairingFakeScreenState();
}

class _HomeKitPairingFakeScreenState extends State<HomeKitPairingFakeScreen> {
  ...
  void initState() {
    ...
    _availableDevices = devices.where((d) => d.integration == IntegrationType.homekit).toList();
    _startDiscovery();
  }
  @override
  void dispose() {
  ...
  String _getGenericName(int index) {
    final types = ['Smart Plug', 'Light Bulb', 'Smart Switch', 'Temperature Sensor', 'Door Lock'];
    return types[index % types.length];
  }
  void _startDiscovery() {
    _discoveryTimer = Timer.periodic(const Duration(seconds: 2, milliseconds: 500), (timer) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Спарювання HomeKit-пристрою'),
      ),
          ...
          child: Row(
            children: [
              SizedBox(
                width: 24,
                height: 24,
                child: CircularProgressIndicator(strokeWidth: 3, valueColor: AlwaysStoppedAnimation<Color>(Colors.teal.shade700)),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    const Text('Пошук HomeKit-пристроїв', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 4),
                    Text('Виявлення пристроїв через mDNS', style: TextStyle(fontSize: 13, color: Colors.grey.shade600)),
                  ],
                ),
              ),
            ],
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: Container(
                          ...
                          child: Icon(Icons.power, color: Colors.teal, size: 28),
                        ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Center(child: Icon(Icons.power, size: 120, color: colorScheme.primary.withValues(alpha: 0.7))),
          ...
          Text('Виявлено через mDNS', style: TextStyle(fontSize: 16, color: Colors.grey.shade600), textAlign: TextAlign.center),
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('PIN-код спарювання', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text('${_selectedDevice!.id.substring(0, 3)}-${_selectedDevice!.id.substring(3, 5)}-${_selectedDevice!.id.substring(5, 8)}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, letterSpacing: 3)),
                    ],
                  ),
                ),
              ],
